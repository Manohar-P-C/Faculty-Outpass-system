...
faculties = [
    ("Dr. Bhagya N P", "Associate Professor & HOD"),
    ("Dr. Raveendra R S", "Associate Professor"),
    ("Prof. Anil Kumara K V", "Assistant Professor"),
    ("Prof. Muthuraju M", "Assistant Professor"),
    ("Prof. Sathyanarayani S", "Assistant Professor"),
]

def add_chem_faculty():
    ...
    hod_email = "bhagyanp.chem@svit.edu"
    cursor.execute("""
        UPDATE hods 
        SET name = %s, email = %s
        WHERE department = 'CHEM'
    """, ("Dr. Bhagya N P", hod_email))
    print("Updated CHEM HOD to Dr. Bhagya N P.")
    ...
    for name, designation in faculties:
        ...
        email = f"{clean_name}.chem@svit.edu"
    ...
    cursor.close()
    conn.close()
    print(f"\nSuccessfully added {count} faculty members to the CHEM department.")
